...
int main(int argc, char **argv)
{
	...
	// Initialize window
	GLFWwindow* window = glfwCreateWindow(800, 600, "2D Deferred Lighting", nullptr, nullptr);
    ...
    spriteRenderProgram->AttachShader(new Shader("../shaders/vertex.glsl", GL_VERTEX_SHADER));
    spriteRenderProgram->AttachShader(new Shader("../shaders/spriteFrag.glsl", GL_FRAGMENT_SHADER));
    Material* spriteMat = new Material(spriteRenderProgram);
    spriteMat->IncRefCount();
    spriteMat->SetTexture("texColor", new Texture("../assets/texture.png"));
    spriteMat->SetTexture("texNormal", new Texture("../assets/normal.png"));
    ...
    lightRenderProgram->AttachShader(new Shader("../shaders/vertex.glsl", GL_VERTEX_SHADER));
    lightRenderProgram->AttachShader(new Shader("../shaders/lightFrag.glsl", GL_FRAGMENT_SHADER));
    Material* lightMat = new Material(lightRenderProgram);
    lightMat->SetTexture("texNormal", screenNormal);
	...
	while (!glfwWindowShouldClose(window))
	{
        ...
        //spriteRenderMat->SetVec3("lightPosition", direction3d);
        ...
        // Loop and queue 192 sprites to be drawn
        for (int i = 0; i < 16; i++)
        {
            for (int j = 0; j < 12; j++)
            {
                spriteBatcher->Draw(glm::vec4(i * 50 , j * 50, 50, 50), glm::vec4(0, 0, 512, 512), glm::vec4(1, 1, 1, 1), spriteMat);
            }
        }
        // Draw sprites
        spriteBatcher->Flush();
